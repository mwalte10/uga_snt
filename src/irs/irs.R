#setwd('Z:/Maggie/uga_snt/src/irs/')
library(tidyverse)
library(ggplot2)
library(data.table)
library(readxl)
...
save = copy(irs)
irs[`NOTE:` == 'DISTRICT COVERAGE', Subcounty := 'ALL']
irs <- irs[Subcounty == 'ALL',.(year = Year, name_2 = Districtname, insecticide = Chemicalused,
...
irs <- irs[year < 2026,]
##if reports over 100% population coverage, change to structure coverage
irs[pop_coverage > 1, pop_coverage := structure_coverage]
irs[is.na(pop_coverage), pop_coverage := structure_coverage] ##only applies to Bugiri
...
irs_adm1 <- copy(irs)
irs_adm1 <- irs_adm1[,.(pop_spray = sum(pop_spray),
                        par_pf = sum(par_pf)), by = c('year',
                                                      'insecticide', 'Campaignwave',
                                                      'Campaignmonth', 'name_1')]
irs_adm1[,pop_coverage := pop_spray / par_pf]
irs <- rbind(irs[,low_level := 'adm_2'],
             irs_adm1[,low_level := 'adm_1'], fill = T)
assertthat::assert_that(all(irs$pop_coverage <= 1))
...
assertthat::assert_that(all(irs$irs_cov <= 1))
...
site_irs_adm1 <-  site_irs_adm1[,.(pop_sprayed = sum(pop_sprayed),
                                   par_pf = sum(par_pf)), by = c('year', 'name_1', 'round', 'insecticide')]


site_irs_adm2[,pop_coverage := pop_sprayed / par_pf]
site_irs_adm1[,pop_coverage := pop_sprayed / par_pf]
...
assertthat::assert_that(all(site_irs$irs_cov <= 1))
...
assertthat::assert_that(all(hybrid$irs_cov <= 1))

##Just for Buekedi years 2018:2019, use the MIS survey value as the MAP estimates don't seem to capture it.
##No evidence of spraying occurring in Busia, so excluding that.
hybrid_bukedi_old <- hybrid[name_1 == 'Bukedi' & year %in% 2018:2019]
hybrid <- hybrid[!(name_1 == 'Bukedi' & name_2 %in% setdiff(loc_map[name_1 == 'Bukedi', name_2], 'Busia') & year %in% 2018:2019)]
hybrid <- hybrid[!(name_1 == 'Acholi' & year %in% 2018:2019)]
bukedi_dhs_mean <- dhs[variable == 'irs_coverage' & SurveyYear == 2018 & adm_1_2025 == 'Bukedi' & admin_level == 1, mean]
bukedi_dhs <- data.table(expand.grid(list(country = 'Uganda', iso3c = 'UGA', name_1 = 'Bukedi',
                                              name_2 = setdiff(loc_map[name_1 == 'Bukedi', name_2], 'Busia'),
                                              year = 2018:2019, peak_season = NA, round = 1, spray_day_of_year = 1, low_level = 'adm_1')))
bukedi_dhs[,irs_cov := bukedi_dhs_mean]
bukedi_dhs_adm1 <-  data.table(country = 'Uganda', iso3c = 'UGA', name_1 = 'Bukedi',
                                                 name_2 = NA,
                                                 year = 2018:2019, peak_season = NA, round = 1,
                               spray_day_of_year = 1, low_level = 'adm_1',
                               irs_cov = bukedi_dhs_mean)
bukedi_dhs <- rbind(bukedi_dhs[,low_level := 'adm_2'],
                    bukedi_dhs_adm1[,low_level := 'adm_1'])
bukedi_dhs[,insecticide := 'actellic']
bukedi_dhs[,variable := 'DHS/MIS']
bukedi_dhs[,source := 'DHS/MIS']

acholi_dhs_mean <- dhs[variable == 'irs_coverage' & SurveyYear == 2018 & adm_1_2025 == 'Acholi' & admin_level == 1, mean]
acholi_dhs <- data.table(expand.grid(list(country = 'Uganda', iso3c = 'UGA', name_1 = 'Acholi',
                                          name_2 = loc_map[name_1 == 'Acholi', name_2],
                                          year = 2018:2019, peak_season = NA, round = 1, spray_day_of_year = 1, low_level = 'adm_1')))
acholi_dhs[,irs_cov := acholi_dhs_mean]
acholi_dhs_adm1 <-  data.table(country = 'Uganda', iso3c = 'UGA', name_1 = 'Acholi',
                               name_2 = NA,
                               year = 2018:2019, peak_season = NA, round = 1,
                               spray_day_of_year = 1, low_level = 'adm_1',
                               irs_cov = acholi_dhs_mean)
acholi_dhs <- rbind(acholi_dhs[,low_level := 'adm_2'],
                    acholi_dhs_adm1[,low_level := 'adm_1'])
acholi_dhs[,insecticide := 'actellic']
acholi_dhs[,variable := 'DHS/MIS']
acholi_dhs[,source := 'DHS/MIS']
...
assertthat::assert_that(all(irs_dt$irs_cov <= 1))
